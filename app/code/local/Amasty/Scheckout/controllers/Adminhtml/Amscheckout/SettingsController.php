...
class Amasty_Scheckout_Adminhtml_Amscheckout_SettingsController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function saveAction(){
        ...
        foreach($saveData as $key => $saveItems){
            ...
            if (strpos($key, 'area') !== FALSE){
                ...
                $this->_setLabelForCustomAttribtue($field->getFieldKey(), $saveItems, $storeId);
            }
        }
    ...
    protected function _setLabelForCustomAttribtue($fieldKey, $saveItems, $storeId)
    {
        if (isset($saveItems['field_label'])) {
            $helperCf = Mage::helper('amscheckout/customfield');
            $area = $helperCf->getPartCustomFieldsName($fieldKey);
            $attrCode  = $helperCf->getPartCustomFieldsName($fieldKey, $area, 'className');
            $attribute = Mage::getSingleton('eav/config')->getAttribute('customer_address', $attrCode);
            if ($helperCf->isCustomField($fieldKey, $area)) {
                if (!$storeId) {
                    $attribute->setFrontendLabel($saveItems['field_label'])->save();
                } else {
                    $labels = array($storeId => $saveItems['field_label']);
