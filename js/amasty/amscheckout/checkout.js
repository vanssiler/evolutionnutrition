...
function ajaxUpdate(url, after, form){
    ...
    if (typeof amastyScheckoutLogin !== 'undefined') {
    	data = $H(data).merge({ 'billing[email]' : amastyScheckoutLogin.email.value});
    }

    var params = form.tagName ? data : form;
...
//    showLoading();
    ...
    return new Ajax.Request(url, {
        ...
        onSuccess: function(response) {
            ...
            if (config.html) {
                if ((config.html.is_ampromo_active != undefined) && config.html.is_ampromo_active) {
                ...
                if (config.html.payment_method && $('checkout-payment-method-load')){
                    $('checkout-payment-method-load').update(config.html.payment_method);
...
function reviewEvents(){
    ...
    $$('.amasty-review-qty').each(function(input){
...
Ajax.Responders.register({
    onComplete: function(response) {
        var config  = response.transport.responseText.evalJSON();
        if (typeof config.html.base_grand_total_updated == 'number') {
            quoteBaseGrandTotal = config.html.base_grand_total_updated;
        }
    }
});