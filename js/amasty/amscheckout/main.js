...
amScheckout.prototype = {
    ...
    initDefData: function(){
        ...
        this.tabsContainer.select("[id=area_content]").each(function(areaContent){
            ...
            areaContent.select("[id=field_row]").each(function(fieldRow){
                ...
                defData['field_' + fieldId] = {
                    ...
                    'field_is_drag': parseInt(fieldRow.getAttribute('field_is_drag')),
                    'use_default': useDefault ? (useDefault.checked ? "1" : "0") : null
                }
            })            
        });
            ...
            Droppables.add(
                row,
                {
                    ...
                    onDrop: function( draggable,droparea){
                        var isDragField  = parseInt(draggable.getAttribute('field_is_drag'));
                        var isEnableDropField = parseInt(droparea.getAttribute('field_is_drag'));
                        if (isDragField && isEnableDropField) {
    ...
    changeOrder: function(draggable, droparea){
        ...
        var isDrag         = parseInt(droparea.getAttribute('field_is_drag'));
    ...
    initEvents: function(){
        ...
        this.tabsContainer.select("a#add_fields").each(function(a){
            a.observe('click', function(){
                ...
                html.push("<br/>");
                html.push("<div>");
                html.push("Need more custom fields? ");
                html.push("</div>");
                html.push("<div>");
                html.push("Please consider <a href='https://amasty.com/order-attributes.html'>Order Attributes</a> extension");
                html.push("</div>");
        ...
        this.tabsContainer.select("input[rel=use_default]").each(function(input){
            ...
            var disable = function(val){
                ...
                var inputDisable = function(dinput){
                    if (dinput != input && dinput.className.indexOf('am-external') == -1){
                        dinput.disabled = val;        
                    }
                }
        ...
        dialog.element.select('button#add_fields')[0].observe('click', function(){
            dialog.element.select("[rel=field_ch]").each(function(checkbox){
                if (checkbox.checked){
                    ...
                    var onlyDragField  = (parseInt(fieldRow.getAttribute('field_is_drag'))) ? true : false;
                    
                    fieldRow.remove();
                    if (onlyDragField) {
                        var lastElement = _caller.getLastDragElement(container);
                        if (lastElement) {
                            lastElement.insert({after: fieldRow});
                        }
                    } else {
                    ...
                    var order          = _caller.getMaxFieldOrder(areaId, onlyDragField) + 100;
    ...
    getMaxFieldOrder: function(areaId, isDragField){
        var onlyDragField = (typeof isDragField != 'undefined' && isDragField == true) ? true : false;
        var maxOrder = 0;
        for (var key in this.defData){
            if (key.indexOf("field_") != -1 &&
                this.defData[key]['area_id'] == areaId) {
                if ((onlyDragField == true && this.defData[key]['field_is_drag'] == 0) ||
                    (onlyDragField && this.defData[key]['field_disabled'] == 1)) {
                    continue;
                }
        ...
        return (onlyDragField) ? parseInt(maxOrder) - 100 : parseInt(maxOrder);
    ...
    getLastDragElement: function(container){
        var lastDragElement = null;
        if ($('am_field_container')) {
            lastDragElement = container.select('div#field_row[field_is_drag="1"]').findAll(function(el){
                return el.visible();
            }).last();
        }
        return lastDragElement;
    },
    initialize: function()
    {
        var _caller = this;
