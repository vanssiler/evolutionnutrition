...
class MGS_Social_Model_Facebook_Client
{
    const REDIRECT_URI_ROUTE = 'social/facebook/connect';

    const XML_PATH_ENABLED = 'social/facebook/active';
    const XML_PATH_CLIENT_ID = 'social/facebook/client_id';
    const XML_PATH_CLIENT_SECRET = 'social/facebook/client_secret';
        ...
        if($response->isError()) {
            ...
            if(($status == 400 || $status == 401)) {
                if(isset($decoded_response->error->message)) {
                    ...
                    $message = Mage::helper('social')
                        ->__('Unspecified OAuth error occurred.');
                }

                throw new MGS_Social_FacebookOAuthException($message);
            } else {
                $message = sprintf(
                    Mage::helper('social')
...
class MGS_Social_FacebookOAuthException extends Exception
{}
