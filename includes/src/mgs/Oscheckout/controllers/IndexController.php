    ...
    public function saveOrderAction() {
        ...
        try {
			...
			if ($orderComment != "")
			{
				...
				$rs = $read->query('update '.$gridTable.' set mgs_order_comment = "'.$orderComment.'" where increment_id = "'.$incrementId.'"');
			}
		...
		foreach ($_shippingRateGroups as $code => $_rates){
			//if(count($_rates)==2){
				foreach ($_rates as $_rate){
					...
					$ShippingMethoddata = $this->getRequest()->getPost('shipping_method', '');
					$save_shippingmethod = $this->getOnepage()->saveShippingMethod($ShippingMethoddata);
					//$this->getOnepage()->getQuote()->getShippingAddress()->setShippingMethod($shipping_method);
					$this->getOnepage()->getQuote()->collectTotals()->save(); 
