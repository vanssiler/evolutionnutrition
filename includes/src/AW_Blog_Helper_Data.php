...
class AW_Blog_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function getBlogUrl($route = null, $params = array())
    {
		...
		$blogRoute = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB) . $blogRoute;
        return $blogRoute;
    }
	public function getPostArrId(){
	...
	public function getNextUrl($id){
		...
		if(is_array($arrId)){
			...
			return $this->getUrlPortById($idNext);
		}
		return;
	}
	...
	/***/
	public function getNextPort($id){
		$arrId = $this->getPostArrId();
		if(is_array($arrId)){
			$maxKey = count($arrId) - 1;
			$key = array_search($id, $arrId);
			...
			$idNext = $arrId[$nextKey];
			$post = Mage::getModel('blog/post')->load($idNext);
			return $post;
		}
		return;
	}
	public function getPrevPort($id){
		$arrId = $this->getPostArrId();
		if(is_array($arrId)){
			$maxKey = count($arrId) - 1;
			$key = array_search($id, $arrId);
